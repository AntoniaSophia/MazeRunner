...
def maze_visualize():
    ...
    executeSript = os.path.join(
        projectDirectory, "Framework", "Visualizer", "maze_visualize.py")
    control.maze_gui_proc = Popen(
        [pythonVar, executeSript], shell=False)  # something long running
...
def maze_solver_loader():
    ...
    if control.team == "":
        ...
        print('Solver Action')
        executeSript = os.path.join(
            projectDirectory, "Teams", control.team, "MazeSolverClient.py")
        control.solver_action_proc = Popen(
            [pythonVar, executeSript], shell=True, stdout=sys.stdout, stderr=sys.stderr)  # something long running
...
def mqtt_broker():
    ...
    if sys.platform == "win32":
        ...
        control.mqtt_broker_proc = Popen(
            [executeSript], shell=False)  # something long running
...
def generator_action(width, height, complexity, density):
    print("w: {} | h: {} | c: {} | d: {}".format(
        width, height, complexity, density))
    ...
    control.generator_action = Popen(['python', executeSript, "--width="+str(width), "--height="+str(
        height), "--complexity="+str(complexity), "--density="+str(density)], shell=False)  # something long running
