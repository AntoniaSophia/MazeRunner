...
class Client(object):
    ...
    def __init__(self, server_ip_port, long_connection=True, verbose=False):
        ...
        server_ip_port should be an ip:port or hostname:port as a
        single string.
        ...
        * In short connection mode, we open a connection when it's needed and
          close it immediately after.  This means creating a connection for
          each call to put_pixels. Keeping the connection usually closed
          makes it possible for others to also connect to the server.

        A connection is not established during __init__.  To check if a
        connection will succeed, use can_connect().

        If verbose is True, the client will print debugging info to the
        console.
    ...
    def can_connect(self):
        ...
        If in long connection mode, this connection will be kept
        and re-used for subsequent put_pixels calls.
    ...
    def set_interpolation(self, enabled=True):
        ...
        if enabled:
            ...
            config_bit = 2
        message = struct.pack('BBBBBBBBB', 0, 255, 0,
                              5, 0, 1, 0, 2, config_bit)
        ...
        try:
            ...
            self._debug(
                'set_interpolation: connection lost.  could not send \
